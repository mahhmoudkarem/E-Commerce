...
export class WishlistComponent implements OnInit {
  ...
  clearWishList():void{
    for (let i = 0; i < this.wishListProducts().length; i++) {
      ...
      this.wishlistService.removeProductFromWishList(this.wishListProducts()[i]._id).subscribe(res=>{ 
        
        this.getAllWishList();
      })
      
    }
    this.toastrService.success( 'WishList has been cleared', 'Exclusive');
